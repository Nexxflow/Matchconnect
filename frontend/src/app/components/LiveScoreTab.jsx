...
const COLOR = {
  get bg() {
    return isLightMode() ? "#f6f9f7" : "#0a0f18";
  },
  get surface() {
  ...
  get purple() {
    return isLightMode() ? "#9333ea" : "#a855f7";
  },
  get purpleGradient() {
    return "linear-gradient(135deg,#a855f7 0%,#ec4899 100%)";
  },
  get red() {
    return isLightMode() ? "#dc2626" : "#ef4444";
  },
  get redGradient() {
...
// Card themes (visual only — cycling palette for lists)
const CARD_THEMES = [
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-4 rounded-2xl relative overflow-hidden" style={cardStyle}>
        ...
        <button
          onClick={onScoreNew}
          className={`w-full sm:w-auto px-4 py-2.5 rounded-xl text-xs font-extrabold uppercase tracking-wider shadow-lg text-center relative z-10 ${BTN_TRANSITION} hover:opacity-95 hover:scale-[1.02]`}
      ...
      {matches !== null && matches.length === 0 && !error && (
        <div className="text-center p-8 rounded-2xl relative overflow-hidden" style={{ border: `2px dashed ${COLOR.border}` }}>
          ...
          <button
            onClick={onScoreNew}
            className={`px-4 py-2 rounded-xl text-xs font-bold ${BTN_TRANSITION} hover:opacity-95 hover:scale-[1.03]`}
            style={{ background: COLOR.accentGradient, color: "#ffffff", boxShadow: "0 6px 20px -6px rgba(34,197,94,0.7)" }}
          >
            Create Match
          </button>
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                      {isCreator && (
                        <button
                          ...
                          className={`px-2.5 py-1.5 rounded-lg text-xs font-bold text-red-400 ${BTN_TRANSITION} hover:scale-105`}
                          style={{ background: "linear-gradient(135deg,rgba(239,68,68,0.15) 0%,rgba(220,38,38,0.15) 100%)", border: "1px solid rgba(239,68,68,0.35)" }}
                        >
                          🗑️ Delete
                        </button>
                      )}
      ...
      {completed.length > 0 && (
        <div className="space-y-3">
          ...
          {completed.map((m, idx) => {
            const isCreator = !m.created_by || (user?.id && String(m.created_by) === String(user.id));
            const ct = CARD_THEMES[(idx + 1) % CARD_THEMES.length];
            return (
              <div
                key={m.id}
                onClick={() => onViewScoreboard(m.id)}
                className={`w-full text-left p-4 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 transition-all cursor-pointer relative overflow-hidden hover:scale-[1.005] ${BTN_TRANSITION}`}
                style={{
                  ...cardStyle,
                  border: `1px solid ${ct.ring}`,
                  boxShadow: `0 6px 22px -16px ${ct.glow}`
                }}
              >
                <div className="absolute left-0 top-0 bottom-0 w-1.5" style={{ background: ct.grad }} />
                <div className="pl-2">
                  <div className="font-bold text-sm mb-1" style={{ color: COLOR.ink }}>{m.team1_name} vs {m.team2_name}</div>
                  <div
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="submit"
            disabled={!canSubmit}
            className={`flex-1 py-2.5 rounded-xl font-bold text-sm text-white disabled:opacity-40 ${BTN_TRANSITION} hover:opacity-95 hover:scale-[1.01]`}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            type="submit"
            disabled={submitting}
            className={`flex-1 py-2.5 rounded-xl font-bold text-sm text-white ${BTN_TRANSITION} hover:opacity-95 hover:scale-[1.01]`}
            style={{ background: COLOR.accentGradient, boxShadow: "0 6px 20px -6px rgba(34,197,94,0.7)" }}
          >
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={submitting}
            className={`flex-1 py-3 rounded-xl font-extrabold text-sm text-white ${BTN_TRANSITION} hover:opacity-95 hover:scale-[1.01]`}
...
function MatchLiveConsole({ user, matchId, onChangeStage, onMatchComplete }) {
  ...
  if (!live.current_innings) {
    if (live.match?.status === "completed") {
      return (
        <div className="p-6 rounded-2xl text-center space-y-3 relative overflow-hidden" style={cardStyle}>
          ...
          <p className="text-xs font-semibold" style={{ color: COLOR.accent }}>{live.match.result || "Match finished"}</p>
          <button
            onClick={() => setActiveTab("scorecard")}
            className={`px-4 py-2 rounded-xl text-xs font-bold text-white ${BTN_TRANSITION} hover:opacity-95 hover:scale-[1.03]`}
            style={{ background: COLOR.accentGradient, boxShadow: "0 6px 20px -6px rgba(34,197,94,0.7)" }}
          >
            View Full Scorecard ➔
          </button>
        ...
        <div className="p-6 rounded-2xl space-y-4 relative overflow-hidden" style={cardStyle}>
          ...
          <div className="text-center space-y-2 pb-4" style={{ borderBottom: `1px solid ${COLOR.border}` }}>
            ...
            <p className="text-xs" style={{ color: COLOR.inkDim }}>
              ...
              <span className="font-mono">{live.first_innings.overs_completed}</span> overs
            </p>
  ...
  if (prompts?.needs_new_batsman) {
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="p-4 rounded-xl flex items-center justify-between relative overflow-hidden" style={{ background: isLightMode() ? "#ffffff" : "#131a26", border: "1px solid rgba(239,68,68,0.4)", boxShadow: "0 6px 22px -12px rgba(239,68,68,0.6)" }}>
  ...
  if (showBowlerPicker) {
    ...
    return (
      <div className="space-y-3">
        <div className="p-3.5 rounded-xl flex items-center justify-between text-xs relative overflow-hidden" style={{ background: isLightMode() ? "#ffffff" : "#131a26", border: "1px solid rgba(59,130,246,0.4)", boxShadow: "0 6px 22px -12px rgba(59,130,246,0.6)" }}>
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {live.chase && (
          <div className="p-3.5 rounded-xl space-y-1.5 relative overflow-hidden" style={{ background: isLightMode() ? "#fffbeb" : "rgba(15,23,42,0.7)", border: "1px solid rgba(245,158,11,0.5)", boxShadow: "0 6px 22px -12px rgba(245,158,11,0.7)" }}>
        ...
        <div className="space-y-4">
              ...
              {recent_balls.map((raw, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ backgroundColor: colorDef.bg, color: colorDef.fg, boxShadow: `0 4px 12px -4px ${colorDef.bg}` }}
                  >
                    {b.val}
                  </div>
                );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
            ...
            {bowling.filter((b) => b.is_current).map((b) => {
              ...
              return (
                <div key={b.player_id} className="p-3.5 rounded-2xl col-span-2 relative overflow-hidden" style={{ background: isLightMode() ? "linear-gradient(135deg,#eff6ff 0%,#f5f3ff 100%)" : "linear-gradient(135deg,#0f1420 0%,#140f1a 100%)", border: `1px solid ${COLOR.blue}`, boxShadow: "0 6px 22px -12px rgba(59,130,246,0.7)" }}>
          ...
          {isCreator && (
            <div className="p-4 rounded-2xl space-y-3 relative overflow-hidden" style={{ ...cardStyle, border: `1px solid ${COLOR.accent}`, boxShadow: "0 8px 26px -14px rgba(34,197,94,0.7)" }}>
              ...
              {isOverEnded && !currentBowler ? (
                <div className="p-4 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-3 relative overflow-hidden" style={{ background: isLightMode() ? "linear-gradient(135deg,#eff6ff 0%,#ecfeff 100%)" : "linear-gradient(135deg,#0f1420 0%,#0a1a1a 100%)", border: `1px solid ${COLOR.blue}`, boxShadow: "0 8px 26px -14px rgba(59,130,246,0.7)" }}>
                  <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background: COLOR.blueGradient }} />
            ...
            {(live.commentary_balls && live.commentary_balls.length > 0 ? live.commentary_balls : recent_balls.slice().reverse()).map((b, idx) => {
              ...
              return (
                <div
                  ...
                  style={{ background: isLightMode() ? "linear-gradient(135deg,#f8fafc 0%,#ffffff 100%)" : "linear-gradient(135deg,#131a26 0%,#1a2333 100%)", border: `1px solid ${COLOR.border}` }}
                >
                  <div className="absolute left-0 top-0 bottom-0 w-0.5" style={{ background: badge.bg }} />
                  <div className="flex flex-col items-center gap-1 shrink-0 pt-0.5 pl-1">
                    <span
                      ...
                      style={{ backgroundColor: badge.bg, color: badge.fg, boxShadow: `0 4px 12px -4px ${badge.bg}` }}
                    >
                      {ballClass.val}
                    </span>
...
function ExtraRunsPicker({ extraType, onCancel, onConfirm }) {
  ...
  return (
    <div className="p-4 rounded-xl space-y-3 relative overflow-hidden" style={{ background: isLightMode() ? "linear-gradient(135deg,#fffbeb 0%,#fef3c7 100%)" : "linear-gradient(135deg,#1a1408 0%,#0f1a12 100%)", border: `1px solid ${COLOR.amber}`, boxShadow: "0 8px 26px -14px rgba(245,158,11,0.7)" }}>
      <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background: COLOR.amberGradient }} />
      <div
...
function WicketPanel({ batting, fieldingSquad, onCancel, onConfirm }) {
  ...
  return (
    <div className="p-4 rounded-xl space-y-4 shadow-xl relative overflow-hidden" style={{ background: isLightMode() ? "linear-gradient(135deg,#fef2f2 0%,#ffffff 100%)" : "linear-gradient(135deg,#1a0f0f 0%,#0f1a12 100%)", border: `1px solid ${COLOR.red}`, boxShadow: "0 8px 26px -14px rgba(239,68,68,0.7)" }}>
      <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background: COLOR.redGradient }} />
...
function OpeningSelectors({ squads, match, onStart, onAddPlayer, busy, inningsNumber = 1 }) {
  ...
  return (
    <div className="p-5 rounded-2xl space-y-5 animate-fadeIn relative overflow-hidden" style={cardStyle}>
      ...
      {error && (
        <div className="text-xs p-3.5 rounded-xl font-semibold flex items-center gap-2 relative overflow-hidden" style={{ background: isLightMode() ? "linear-gradient(135deg,#fef2f2 0%,#fee2e2 100%)" : "linear-gradient(135deg,rgba(239,68,68,0.15) 0%,rgba(220,38,38,0.15) 100%)", border: `1px solid ${COLOR.red}`, color: COLOR.red }}>
...
function PlayerPicker({ title, players, onPick, onAddNew, busy }) {
  ...
  return (
    <div className="p-5 rounded-2xl space-y-4 relative overflow-hidden" style={cardStyle}>
      ...
      {pickerError && (
        <div className="text-xs p-3 rounded-lg font-bold relative overflow-hidden" style={{ background: isLightMode() ? "linear-gradient(135deg,#fef2f2 0%,#fee2e2 100%)" : "linear-gradient(135deg,rgba(239,68,68,0.15) 0%,rgba(220,38,38,0.15) 100%)", border: `1px solid ${COLOR.red}`, color: COLOR.red }}>
          <div className="absolute left-0 top-0 bottom-0 w-1" style={{ background: COLOR.redGradient }} />
...
function FinalScoreboard({ matchId }) {
  ...
  return (
    <div className="space-y-4">
      <div className="p-5 rounded-2xl text-center relative overflow-hidden" style={cardStyle}>
        <div className="absolute top-0 left-0 right-0 h-1" style={{ background: "linear-gradient(90deg,#f59e0b,#ec4899,#a855f7)" }} />
        ...
        <div
          className="text-lg font-black flex items-center justify-center gap-2 relative z-10"
          style={{
            background: "linear-gradient(135deg,#f59e0b 0%,#ec4899 100%)",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
