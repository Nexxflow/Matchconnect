...
function GroundForm({ token, onCreated }) {
  const emptyForm = {
    name: "",
    address: "",
    contact_no: "",
  ...
  const handleSubmit = async e => {
    ...
    if (!/^[0-9]{10,15}$/.test(form.contact_no.trim())) return setError("Enter a valid contact number (10-15 digits).");
    if (!form.price || Number(form.price) <= 0) return setError("Price per hour must be greater than 0.");
    if (!token) {
      return setError("You need to be logged in to register a ground.");
    }
    setSubmitting(true);
    try {
      ...
      // Normalize the row the backend just returned so it matches the
      // shape every other ground in the list is rendered with.
      // Postgres numeric columns come back as strings (e.g. "1200.00"),
      // and freshly-created grounds won't have rating/amenities/tags yet —
      // without this, rendering the new item can throw and blank the page.
      const normalized = {
        ...res.ground,
        price: res.ground.price != null ? Number(res.ground.price) : null,
        rating: res.ground.rating != null ? Number(res.ground.rating) : 0,
        amenities: res.ground.amenities ?? [],
        tags: res.ground.tags ?? []
      };
  ...
  return <form onSubmit={handleSubmit} className={cn(C, "rounded-2xl p-4 space-y-3")}>
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-white">Register a Ground</span>
        <button type="button" onClick={() => { setOpen(false); setError(null); }} className="w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: "#222" }}>
          <X className="w-3.5 h-3.5 text-[#c8ccc8]" />
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2">
          <label className="text-xs mb-1 block" style={{ color: "#6b7a6b" }}>Address</label>
          <input value={form.address} onChange={e => update("address", e.target.value)} className="w-full rounded-xl px-3 py-2 text-sm text-white focus:outline-none" style={{ backgroundColor: "#111", border: "1px solid #2a2a2a" }} placeholder="Linking Road, Bandra West" />
        </div>
        <div>
          <label className="text-xs mb-1 block" style={{ color: "#6b7a6b" }}>Contact number</label>
          <input value={form.contact_no} onChange={e => update("contact_no", e.target.value)} className="w-full rounded-xl px-3 py-2 text-sm text-white font-mono focus:outline-none" style={{ backgroundColor: "#111", border: "1px solid #2a2a2a" }} placeholder="9876543210" />
        </div>
        <div>
          <label className="text-xs mb-1 block" style={{ color: "#6b7a6b" }}>Price per hour (₹)</label>
          <input type="number" min="1" value={form.price} onChange={e => update("price", e.target.value)} className="w-full rounded-xl px-3 py-2 text-sm text-white font-mono focus:outline-none" style={{ backgroundColor: "#111", border: "1px solid #2a2a2a" }} placeholder="1200" />
        </div>
      </div>
      ...
      <button type="submit" disabled={submitting} className="w-full py-2.5 rounded-xl bg-green-500 text-black font-bold text-sm hover:bg-green-400 transition-colors" style={submitting ? { opacity: 0.6, cursor: "not-allowed" } : {}}>
        {submitting ? "Registering..." : "Register Ground"}
      </button>
    </form>;
}

function GroundsTab({ onBook, grounds = GROUNDS, token, onGroundCreated }) {
  ...
  // price may arrive as a Postgres numeric string ("1200.00") or a preset
  // display string ("₹1200/hr") in the mock data — handle both safely.
  ...
  // amenities/tags may come back from the API as a JSON string (TEXT column)
  // rather than an already-parsed array (JSONB column), or be missing
  // entirely on freshly-created grounds — normalize all cases safely.
  const asArray = v => {
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      {g.contact_no && <div className="flex items-center gap-1 mt-0.5">
                          <Phone className="w-3 h-3" style={{ color: "#4a5a4a" }} />
                          <span className="text-xs font-mono" style={{ color: "#6b7a6b" }}>{g.contact_no}</span>
                        </div>}
                    </div>
