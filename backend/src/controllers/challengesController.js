...
const listChallenges = asyncHandler(async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT c.*, t.name AS team_name, t.rating AS team_rating, t.wins, t.losses, g.name AS ground_name
     FROM challenges c
...
// POST /api/challenges  Body: { team_id, format, match_date, time_slot, ground_id, urgent, note }
const createChallenge = asyncHandler(async (req, res) => {
  const { team_id, format, match_date, time_slot, ground_id, urgent, note } = req.body;
  if (!team_id || !format || !match_date || !time_slot) {
    return res.status(400).json({ error: "team_id, format, match_date and time_slot are required" });
  }
  const result = await pool.query(
    `INSERT INTO challenges (team_id, format, match_date, time_slot, ground_id, urgent, note)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
    [team_id, format, match_date, time_slot, ground_id || null, urgent || false, note || null]
  );
  res.status(201).json({ challenge: result.rows[0] });
});

// POST /api/challenges/:id/accept  Body: { team_id }
const acceptChallenge = asyncHandler(async (req, res) => {
  ...
  const result = await pool.query(
    `UPDATE challenges SET status = 'accepted', accepted_by_team_id = $1
...
const cancelChallenge = asyncHandler(async (req, res) => {
  const result = await pool.query(
    `UPDATE challenges SET status = 'open', accepted_by_team_id = NULL
